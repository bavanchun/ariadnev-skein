...
final class SourcePIDCache {
    ...
    private struct State {
        ...
        private func stableBounds(for window: MenuBarItemService.ItemWindow) -> CGRect? {
            var cachedBounds = window.bounds

            for n in 1...5 {
                guard let currentBounds = Bridging.getWindowFrame(for: window.windowID) else {
        ...
        mutating func updatePID(for window: MenuBarItemService.ItemWindow) {
            guard
    ...
    private lazy var cancellable = NSWorkspace.shared.publisher(for: \.runningApplications).sink { [weak self] runningApps in
        ...
        Logger.sourcePIDCache.debug("Received new running applications")
    ...
    func start() {
        Logger.sourcePIDCache.debug("Starting observers for source PID cache")
        _ = cancellable
    }

    /// Returns the cached process identifier for the given window,
    /// updating the cache if needed.
    func pid(for window: MenuBarItemService.ItemWindow) -> pid_t? {
        state.withLock { state in
...
private extension Logger {
    static let sourcePIDCache = Logger(category: "SourcePIDCache")
}
